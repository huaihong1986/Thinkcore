package com.thinkcore.view.autolayout.attr;

import android.view.View;

import com.thinkcore.utils.log.TLog;
import com.thinkcore.view.autolayout.utils.AutoUtils;
...
public abstract class AutoAttr {
    private final String Tag = AutoAttr.class.getSimpleName();
 ...
 */
    ...
    public void apply(View view) {
        ...
        if (log) {
            TLog.e(Tag, " pxVal = " + pxVal + " ," + this.getClass().getSimpleName());
        }
        int val;
        if (useDefault()) {
            val = defaultBaseWidth() ? getPercentWidthSize() : getPercentHeightSize();
            if (log) {
                TLog.e(Tag, " useDefault val= " + val);
            }
        } else if (baseWidth()) {
            val = getPercentWidthSize();
            if (log) {
                TLog.e(Tag, " baseWidth val= " + val);
            }
        } else {
            val = getPercentHeightSize();
            if (log) {
                TLog.e(Tag, " baseHeight val= " + val);
            }
        }
