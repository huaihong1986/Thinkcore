...
public class TBase64OutputStream extends FilterOutputStream {
    private final TBase64.Coder coder;
     ...
     * @param flags bit flags for controlling the encoder; see the constants in {@link TBase64}
     */
    public TBase64OutputStream(OutputStream out, int flags) {
        this(out, flags, true);
    }
     ...
     */
    public TBase64OutputStream(OutputStream out, int flags, boolean encode) {
        super(out);
        this.flags = flags;
        if (encode) {
            coder = new TBase64.Encoder(flags, null);
        } else {
            coder = new TBase64.Decoder(flags, null);
        }
    }
    ...
    private void internalWrite(byte[] b, int off, int len, boolean finish) throws IOException {
        ...
        if (!coder.process(b, off, len, finish)) {
            throw new TBase64DataException("bad base-64");
        }
