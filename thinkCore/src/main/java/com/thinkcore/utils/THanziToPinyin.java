...
public class THanziToPinyin {
    ...
    private static THanziToPinyin that;
    ...
    protected THanziToPinyin(boolean hasChinaCollator) {
        this.hasChinaCollator = hasChinaCollator;
    }

    public static THanziToPinyin getInstance() {
        synchronized (THanziToPinyin.class) {
            if (that != null) {
                return that;
            }
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    ...
                    that = new THanziToPinyin(true);
                    return that;
                }
            }
            Log.w(TAG,
                    "There is no Chinese collator, HanziToPinyin is disabled");
            that = new THanziToPinyin(false);
            return that;
        }
    }
